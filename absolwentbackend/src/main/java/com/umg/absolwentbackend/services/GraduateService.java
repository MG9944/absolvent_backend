...
public class GraduateService {
    ...
    public Graduate addGraduate(String name, String lastName, String email, int graduation_year, String faculty, String field, Date date_of_birth, String title,String group,String gender) throws AuthenticationException {
        Integer count = graduateRepository.countbyemail(email);
        if(count>0) {
            throw new AuthenticationException("Graduate already exists");
        }
        Integer userid = graduateRepository.insertGraduate(name,lastName,email,graduation_year,faculty,field,date_of_birth,title,group,gender);
